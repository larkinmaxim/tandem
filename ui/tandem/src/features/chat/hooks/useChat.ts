  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        // Ensure the session is prepared in the tracker before sending.
        // We call this if not already prepared, or if promoting a draft or
        // changing the model, to ensure the volatile sessionTracker state is
        // up to date (e.g. after a page refresh). acpPrepareSession handles
        // caching internally to avoid redundant backend calls when 
        // workingDir/providerId match.
        if (!gooseSessionId || wasDraft || selectedModelId) {
