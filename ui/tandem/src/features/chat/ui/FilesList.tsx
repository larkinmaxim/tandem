...
export function FilesList({ projectWorkingDirs }: FilesListProps) {
  ...
  const handleOpenFile = useCallback(
    async (path: string) => {
      ...
      // List of extensions to open in internal viewer
      const viewableExtensions = [
        ".md", ".txt", ".csv", ".json", ".yaml", ".yml", ".toml",
        ".js", ".jsx", ".ts", ".tsx", ".py", ".rb", ".go", ".rs",
        ".html", ".css", ".scss", ".sql", ".sh", ".bash", ".zsh", ".bat"
      ];
      
      const lowerPath = path.toLowerCase();
      const isViewable = viewableExtensions.some(ext => lowerPath.endsWith(ext));

      if (isViewable) {
        try {
          const content = await readTextFile(path);
          useArtifactStore.getState().openArtifact(content, path);
          return;
        } catch (error) {
          console.error("Failed to read file for preview:", error);
          // Fallback to system open if internal read fails
        }
      }
